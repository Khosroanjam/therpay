...
Window {
    ...
    // Loader برای بارگذاری صفحات
    Loader {
        id: pageLoader
        anchors.fill: parent
        source: ""  // در ابتدا خالی است
        visible: false  // در ابتدا مخفی است
        opacity: 0

        onLoaded: {
            if (item && item.hasOwnProperty("backend")) {
                item.backend = backend;
            }
        }
    }

    // محتوای اصلی صفحه ورود
    Item {
        id: loginPage
        anchors.fill: parent
        visible: true  // در ابتدا نمایان است
                ...
                ColumnLayout {
                    ...
                    Item {
                        ...
                        Button {
                            ...
                            onClicked: {
                                if (backend) {
                                    backend.validateOperatorCode(modernTextField.text)
                                } else {
    ...
    Connections {
        ...
        function onLoginSuccessful() {
            ...
            // بارگذاری صفحه App.qml در Loader
            pageLoader.source = "App.qml"
            pageLoader.visible = true
            root.visible = false
            // انیمیشن محو شدن صفحه ورود
            loginPageFadeOut.start()
        }
    ...
    NumberAnimation {
        ...
        onFinished: {
            ...
            pageLoaderFadeIn.start()
        }
    }

    // انیمیشن ظاهر شدن صفحه جدید
    NumberAnimation {
        id: pageLoaderFadeIn
        target: pageLoader
        property: "opacity"
